...
"""This module contains functions for deleting duplicate nodes."""
...
class CleanupThread(threading.Thread):
    ...
    def run(self):
        ...
        This function calls the delete_duplicate_node function, which deletes duplicate nodes 
        created. It also deletes the entry for the database and the node object.
...
def delete_duplicate_node():
    ...
    sql = "select service_tag from cluster.nodeinfo group by service_tag having (count(*) > 1)"
    cursor.execute(sql)
    service_tag_output = cursor.fetchall()

    for i, tag in enumerate(service_tag_output):
        if tag[0] is not None:
            service_tag_output[i] = str(tag[0])
            print(f"Duplicate service tag: {service_tag_output[i]}")

            # Fetch the node name for duplicate node
            sql = f"""
            select node from cluster.nodeinfo 
            where service_tag='{service_tag_output[i]}' 
            and admin_mac is NULL 
            ...
            if node_output is not None:
                ...
                sql = f"""
                delete from cluster.nodeinfo where node='{node_name}' 
                and service_tag='{service_tag_output[i]}' and admin_mac is NULL
                """
                cursor.execute(sql)

                # Delete the entry from /etc/hosts
                command = ['/opt/xcat/sbin/makehosts', '-d', node_name]
                _ = subprocess.run(command, shell=False, check=False)
...
def main():
    ...
    This function creates an instance of the CleanupThread class and sets its Daemon
    attribute to True. It then starts the CleanupThread.

    If an exception occurs during the execution of the CleanupThread, the exception is caught
    and the message "Exception thrown by the cleanup thread: <exception message>" is printed.
    ...
    """
    try:
        cleanup_thread_object = CleanupThread()
        cleanup_thread_object.daemon = True
        cleanup_thread_object.start()
