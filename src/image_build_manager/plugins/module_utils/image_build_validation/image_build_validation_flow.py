...
def validate_credentials_logic(cred_data, config_data, errors, logger=None):
    ...
    - s3_secret_key is always required (enforced by L1 schema).
    - aarch64_ssh_password is required only when aarch64_inventory_host_ip is set.
    """
    s3 = config_data.get("s3_configurations", {})
    provider = s3.get("provider", "")
    ...
    if aarch64_ip and aarch64_ip.strip():
        ...
        if not aarch64_pw or not aarch64_pw.strip():
            msg = ("image_build_credentials: aarch64_ssh_password is required when "
                   "aarch64_inventory_host_ip is set.")
            errors.append(msg)
            if logger:
                logger.error(msg)
