...
#   3. Creates Ansible log directory:  /var/log/omnia/utils/
#   4. Copies input template files from source tree to runtime data path
#
# Source (flat):   src/utils/input/
# Destination:     <OMNIA_DATA_PATH>/utils/input/<project>/
#
...
#   - collect.ini             (node inventory for log collection)
#   - collect_config.yml      (log collection configuration)
#   - set_pxe_boot.ini        (BMC IPs for PXE boot)
#   - set_pxe_boot_credentials.yml (BMC credentials - placeholder)
#
# Usage:
...
#   cp -a input/*.yml input/*.ini /opt/omnia/utils/input/project_default/
# =============================================================================
