...
class AdapterPolicyService:
    ...
    def __init__(self, settings=None):
        ...
        self.custom_policy_path = (
            self.gui_dir / "backend" / "resources"
            / "adapter_policy_custom.json"
        )
        self.default_policy_path = (
            self.build_stream_dir / "core" / "catalog"
            / "resources" / "adapter_policy_default.json"
        )

        logger.debug(
            "AdapterPolicyService initialized with "
            "custom=%s, default=%s",
            self.custom_policy_path,
            self.default_policy_path,
        )
    ...
    def __repr__(self) -> str:
        return (
            f"AdapterPolicyService("
            f"custom={self.custom_policy_path})"
        )
    ...
    def get_adapter_policy(self) -> Dict[str, Any]:
        ...
        """
        for source, policy_path in [
            ("custom", self.custom_policy_path),
            ("default", self.default_policy_path),
        ]:
            try:
                policy_data = read_json(policy_path)
                logger.debug(
                    "Loaded %s adapter policy from %s (%d keys)",
                    source,
                    policy_path,
                    len(policy_data)
                ...
                logger.exception(
                    "Failed to load %s adapter policy", source,
                )
                raise AdapterPolicyNotFoundError(
                    source, details={"error": str(e)},
                ) from e

        raise AdapterPolicyNotFoundError(
            "default", details={"error": "No policy file found"},
        )
    ...
    def save_adapter_policy(self, policy: Dict[str, Any]) -> None:
        ...
        """
        self.custom_policy_path.parent.mkdir(
            parents=True, exist_ok=True,
        )
        try:
            write_json(self.custom_policy_path, policy)
            logger.info(
                "Saved adapter policy to %s",
                self.custom_policy_path,
            )
        except OSError:
    ...
    def delete_adapter_policy(self) -> None:
        ...
        try:
            ...
            logger.info(
                "Deleted custom adapter policy: %s",
                self.custom_policy_path,
            )
        except FileNotFoundError:
            logger.warning(
                "Custom adapter policy does not exist, "
                "nothing to delete",
            )
