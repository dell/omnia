...
def validate(
    ...
    # Resolve the same strict catalog context used by download execution.
    ...
    cluster_os_version = catalog_context["os_version"]
    ...
    if cluster_os_version not in repositories:
        errors.append(create_error_msg(
            ...
            "Catalog RHEL version is not configured under repositories"
        ))
...
def _validate_catalog_registry_mapping(config_data, logger, _omnia_base_dir,
    ...
    seen_images = set()
    for selected in _iter_selected_packages(
            catalogs, catalog_context["architectures"], logger,
