...
|----------|---------|-------------|
| `Delete_sinks_volume` / `delete_sinks_volume` | `false` | Valid only with the full `cleanup` tag. When `true`, delete sink component PVCs (Kafka, VictoriaMetrics, VictoriaLogs) and Kafka identity metadata. When `false`, preserve sink persistent data and the metadata required for safe redeployment. Source component PVCs (currently iDRAC and PowerScale) are always deleted. |
