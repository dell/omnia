...
This module can be used in the telemetry domain's setup role as a
common precheck.
"""
...
description:
  ...
  - Cross-validates env vars against actual system details
    (hostname, IP on NIC, paths).
...
- name: Validate system environment
  omnia.telemetry.validate_system_environment:
    required_vars:
      - SYSTEM_ADMIN_NIC_IPV4
      - SYSTEM_HOSTNAME
      - SYSTEM_DOMAIN_NAME
...
def main() -> None:
    ...
    env_path = os.environ.get("OMNIA_DATA_PATH", "")
