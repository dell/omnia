...
"""
Ansible module for build_stream-specific input validation.

Performs L1 (JSON schema) and L2 (cross-field logic) validation on:
  - build_stream_config.yml
  - build_stream_credentials.yml (if exists and decrypted)

Usage in a playbook:
  - name: Validate build stream configuration
    validate_build_stream_config:
...
from ansible.module_utils.input_validation.core.validation_engine import (
    logic as validate_build_stream_config,
...
DOCUMENTATION = r'''
---
module: validate_build_stream_config
short_description: Validate build stream configuration files
version_added: "3.0.0"
description:
  - Performs L1 (JSON schema) validation on build_stream_config.yml and
...
EXAMPLES = r'''
- name: Validate build stream configuration
  omnia.build_stream.validate_build_stream_config:
    input_project_dir: /opt/omnia/build_stream/input
    schema_dir: "{{ role_path }}/../../../plugins/module_utils/input_validation/schema"
  register: validation
...
def run_module():
    ...
    for vf in VALIDATION_FILES:
        ...
        if vf["schema_file"] == "build_stream_config.json":
            config_data = data

    # --- L2: Cross-field logic validation ---
    if config_data:
        l2_errors = validate_build_stream_config(config_data, logger)
        if l2_errors:
        ...
        cred_path = os.path.join(
            input_project_dir, "build_stream_credentials.yml"
        )
    ...
    message = [
        f"Build stream configuration validation {status}.",
