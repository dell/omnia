...
├── setup_env.sh                          # Dependency, venv, and credential setup
├── test_config.yml                       # Target server, dataset, sync, report settings
├── test_creds.yml                        # Test-runner access credentials (Vault-encrypted)
...
│   │   │   └── <domain_name>_config.yml  # Non-sensitive input only
...
```
<effective-domain-data-path>/input/<project_name>/
```

The effective root is the non-empty component-specific `*_DATA_PATH` override
when that domain defines one; otherwise it is
`$OMNIA_DATA_PATH/<domain_name>`.
...
|------|-------|--------------------------|-------------|
| `--set-creds` | Test access | No | Interactively create credentials; every secret is entered twice |
| `--update-creds` | Test access | No | Interactively update an existing valid credential store |
| `--creds-stdin` | Test access | No | Read one OIM SSH password from standard input |
| `--set-domain-creds` | Domain | **No** | Interactively create domain credentials; every secret is entered twice |
| `--update-domain-creds` | Domain | **No** | Interactively update an existing valid domain credential store |
| `--domain-creds-stdin` | Domain | **No** | Read a validated JSON object from standard input |

Secret values MUST NOT be accepted as command-line argument values because
process arguments can be visible to other users. Automation pipes a password
or JSON object to the corresponding `*-stdin` flag instead.

`test_creds.yml` is limited to credentials the test runner uses directly.
Domains that need playbook/runtime credentials keep a separate encrypted
domain credential pair. Main has no separate domain credential store. Image
Build Manager and Telemetry resolve their domain root from the non-empty
`IMAGE_BUILD_MANAGER_DATA_PATH` or `TELEMETRY_DATA_PATH` override; when the
override is unset or empty, they use `$OMNIA_DATA_PATH/<domain>`. Both require
`OMNIA_PROJECT_NAME`, and neither silently invents a data root or project.
...
<effective-domain-data-path>/input/<project_name>/
        ...
        v
<effective-domain-data-path>/output/<project_name>/
```
...
| **Input sync dest** | `<effective-domain-data-path>/input/<project>/` | Uses a non-empty domain override, otherwise `OMNIA_DATA_PATH/<domain>` |
