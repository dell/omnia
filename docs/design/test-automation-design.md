...
│           │   └── <domain_name>_credentials.yml.j2
│           └── <upstream>_output/
...
| From src | `python generate_dataset.py <name> --from-src` | Copy from `src/` + generate placeholder credentials |

### 4.5 Generator Output

The generator creates a complete dataset directory with:
- All rendered YAML config files
- Placeholder credentials with empty values
...
5. Credentials in generated datasets MUST have empty placeholder values — never real secrets
