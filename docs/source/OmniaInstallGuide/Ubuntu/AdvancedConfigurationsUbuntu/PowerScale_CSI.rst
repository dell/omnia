...
* For an unsuccessful driver installation scenario, the user first needs to follow the `removal <#removal>`_ steps manually on the ``kube_control_plane`` and then re-run the ``omnia.yml`` playbook for CSI driver installation.
