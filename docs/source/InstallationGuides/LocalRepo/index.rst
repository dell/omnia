...
2. Enter the required values in the ``input/local_repo_config.yml`` file:
    ...
    |                         | * When ``repo_config`` is always, the given list will be configured on the control plane and packages required for cluster will be downloaded.                                                       |
    |      ``JSON List``      | * When ``repo_config`` is partial or never, the given list will not be downloaded.                                                                                                                   |
    |                         | * 'url' defines the baseurl for the repository.                                                                                                                                                      |
    |      Optional           | * 'gpgkey' defines gpgkey for the repository. If 'gpgkey' is omitted then   gpgcheck=0 is set for that repository.                                                                                   |
