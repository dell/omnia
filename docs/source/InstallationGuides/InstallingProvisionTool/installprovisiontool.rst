...
2. Provided that the ``pxe_mapping_file_path`` is updated as per the provided `template (omnia/examples/pxe_mapping_file.csv) <../samplefiles.html>`_, Omnia deploys the control plane and assigns the component roles by executing the ``omnia.yml`` file.  To deploy the Omnia control plane, run the following command ::
