...
interface OsSubscriptionSectionProps {
  osType: 'rhel' | 'ubuntu';
  enabled: boolean;
...
export const OsSubscriptionSection = ({
  osType,
  enabled,
  x86Fields,
  aarch64Fields,
  ...
  removeAarch64,
  onToggle,
}: OsSubscriptionSectionProps) => {
  const prefixX86 = `${osType}_subscription_repo_config_x86_64`;
  const prefixAarch64 = `${osType}_subscription_repo_config_aarch64`;
  const label = `${osType.toUpperCase()} Subscription`;
  ...
  return (
    <>
      <div className="form-group">
        <label className="form-label">{label} Repository Configuration (Optional)</label>
        <div className="form-checkbox">
          <input
            type="checkbox"
            id={`enable-${osType}-subscription`}
            checked={enabled}
            onChange={(e) => onToggle(e.target.checked)}
          />
          <label htmlFor={`enable-${osType}-subscription`}>Enable {label} Repository Configuration</label>
        </div>
      </div>
      ...
      <div className={!enabled ? 'disabled-section' : ''}>
        <div className="space-y-2 section-style">
          ...
          {x86Fields.map((field, index) => renderRepoFields(prefixX86, field.id, index, removeX86, x86Fields.length))}
          ...
          {aarch64Fields.map((field, index) => renderRepoFields(prefixAarch64, field.id, index, removeAarch64, aarch64Fields.length))}
