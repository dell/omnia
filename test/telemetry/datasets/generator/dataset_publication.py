...
def dataset_lock(datasets_dir: Path) -> Iterator[None]:
    """Serialize compare/publish transactions using a lock file.

    Uses a simple file-based lock that works on both Linux and Windows.
    """
    lock_path = datasets_dir / ".generator.lock"
    lock_fd = None
    try:
        try:
            lock_fd = os.open(
        ...
        try:
            ...
            cleanup_errors.append(f"unlink failed: {exc}")
...
def directory_changes(expected: Path, actual: Path) -> list[str]:
    """Return structural and content changes between two directory trees."""
