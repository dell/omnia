...
"""Create the manifest and customer handoff for a generated telemetry dataset."""
...
def external_inputs(
    ...
    inputs: list[str] = [
        "Orchestrator inventory file referenced by cluster_inventory",
    ]
    ...
    if idrac.get("metrics_enabled", False):
        inputs.append(
            "BMC group CSV file referenced by "
            "idrac_telemetry_configurations.bmc_group_data_path"
        )
    ufm = sources.get("ufm", {})
    if ufm.get("metrics_enabled", False):
        inputs.append("Reachable UFM appliance endpoint")
    vast = sources.get("vast", {})
    if vast.get("metrics_enabled", False):
        inputs.append("Reachable VAST cluster endpoint")
    ps = sources.get("powerscale", {})
    if ps.get("metrics_enabled", False):
        inputs.append(
            "CSM Observability values.yaml file for PowerScale"
        )
    packages = documents["telemetry_packages"]
    if packages.get("install_mode", "offline") == "offline":
        inputs.append("Reachable Pulp offline repository (repo_url)")
    return inputs
...
def regeneration_command(plan: dict[str, Any]) -> str:
    ...
    if plan["mode"] == "from-src":
        ...
        for assignment in plan.get("set_values", []):
            command.extend(["--set", assignment])
        for assignment in plan.get("legacy_values", []):
...
def write_readme(
    ...
    lines = [
        ...
        "",
        "Generated from canonical `src/telemetry` input examples by",
...
def _usage_sections(plan: dict[str, Any], regenerate: str) -> list[str]:
    """Return review, regeneration, and execution instructions."""
    dataset = plan["dataset"]
    return [
        "Credentials for OME, SFM, UFM, and VAST appliances are stored in",
        "`test_creds.yml` and auto-encrypted with Ansible Vault on first run.",
        "Never place credentials inside a dataset.",
        "",
        "## Regenerate",
        "",
        "From `test/telemetry`:",
        ...
        "## Use",
        "",
        f"Set `dataset: \"{dataset}\"` in `test_config.yml`. Enable",
        "`sync_telemetry_input: true` to push the dataset's `input/` files",
        "to the target server before running the playbook.",
        "",
    ]
