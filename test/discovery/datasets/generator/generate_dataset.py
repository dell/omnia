...
"""
Dataset Generator — Copies source files into test dataset directories.

Copies discovery input files from src/discovery/input/ into dataset directories.
This provides a simple way to create test datasets without template rendering.
...
# src/ paths (4 levels up from generator/ → test/discovery/datasets/generator → repo root)
REPO_ROOT = GENERATOR_DIR.parents[3]
SRC_INPUT_DIR = REPO_ROOT / "src" / "discovery" / "input"
...
def _list_profiles():
    ...
    print("\nAvailable profiles:")
    print("-" * 50)
    print(f"  {_CYAN}{'defaults':20s}{_NC} (base profile — use with --from-src)")
    for name, data in _available_profiles():
        ome_ip = data.get("ome_ip", "—")
        print(f"  {_CYAN}{name:20s}{_NC} ome_ip={ome_ip}")
    print()
...
def _copy_from_src(output_dir):
    """Copy dataset files directly from src/discovery/input/ directory."""
...
def _generate_readme(dataset_name, profile_name, variables, rendered, output_dir):
    ...
    lines = [
        ...
        f"using source files from `src/discovery/input/`.",
        "",
        "---",
        "",
        "## Source",
        "",
        "This dataset was created by copying files directly from the source:",
        "",
        "| File | Source |",
        "|------|--------|",
        "| `discovery_config.yml` | `src/discovery/input/discovery_config.yml` |",
        "| `network_spec.yml` | `src/discovery/input/network_spec.yml` |",
        "",
        "## Configuration",
        "",
        "| Parameter | Value |",
        "|-----------|-------|",
        f"| Dataset | `{dataset_name}` |",
        f"| ome_ip | `{variables.get('ome_ip', '')}` |",
        f"| ome_username | `{variables.get('ome_username', '')}` |",
        f"| discovery_mechanism | `{variables.get('discovery_mechanism', '')}` |",
        "",
        "## Generated Files",
        "",
        "```",
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate test dataset from source files.",
...
""",
    ...
    parser.add_argument(
        "--from-src",
        action="store_true",
        help="Copy files directly from src/discovery/input/",
    )
    ...
    if not args.from_src:
        parser.error("--from-src is required for discovery dataset generation")
    ...
    print()
    print("=================================================================")
    print(f"  Dataset Generator — {dataset_name} (from src)")
    print("=================================================================")
    print()
    ...
    _info("Copying from src/discovery/input/")
    rendered = _copy_from_src(output_dir)
    
    # Load variables for README generation
    variables = _load_yaml(PROFILES_DIR / "defaults.yml")
    ...
    _generate_readme(dataset_name, "from-src", variables, rendered, output_dir)
