...
build_pytest_args() {
    ...
    # Custom --marker option for quality filtering
    if [[ -n "$MARKER" ]]; then
...
case "$SCENARIO" in
    ...
    help|--help|-h|"")
        echo -e "${BLUE}=================================================================${NC}"
        echo -e "${BLUE}  Discovery — Validation Runner${NC}"
        echo -e "${BLUE}=================================================================${NC}"
        echo ""
        echo "Usage:"
        ...
        echo "  --suite <name>    Filter by subfolder (status, output, etc.)"
        ...
        echo "  discovery    Full end-to-end (deploy + verify outputs)"
        ...
        echo ""
        echo "  # Full discovery run + verify outputs"
        echo "  $0 discovery test"
        echo ""
        echo "  # Verify only output files (no playbook)"
        echo "  $0 discovery verify --suite output"
        echo ""
