...
"""
Pytest configuration for discovery FVT.
...
def _apply_dataset_overrides(config):
    ...
    if si_override:
        config["sync_discovery_input"] = si_override.lower() == "true"
...
def pytest_sessionstart(session):
    ...
    if config.get("sync_discovery_input", False):
        sync_result = sync_discovery_input(host)
        if sync_result["success"]:
            log(sync_result["details"], "OK")
        else:
    ...
    valid_scenarios = {"discovery", "validate"}
    ...
    report = TestReport(
        ...
        report_name=str(config.get("report_name", "discovery_test_report")),
