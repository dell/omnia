...
Examples:
    ...
    python generate_dataset.py my_custom defaults --var gitlab_host=10.5.0.100
...
# src/ paths (4 levels up from generator/ → test/build_stream/datasets/generator → repo root)
REPO_ROOT = GENERATOR_DIR.parents[3]
SRC_INPUT_DIR = REPO_ROOT / "src" / "build_stream" / "input"
...
def _list_profiles():
    ...
    for name, data in _available_profiles():
        gitlab = data.get("gitlab_host", "—")
        print(f"  {_CYAN}{name:20s}{_NC} gitlab_host={gitlab}")
    print()
...
def _copy_from_src(output_dir):
    """Copy dataset files directly from src/build_stream/input/ directory."""
...
def _generate_readme(dataset_name, profile_name, variables, rendered, output_dir):
    ...
    lines = [
        ...
        f"| enable_build_stream | `{variables.get('enable_build_stream', '')}` |",
        f"| build_stream_host_ip | `{variables.get('build_stream_host_ip', '')}` |",
        f"| gitlab_host | `{variables.get('gitlab_host', '')}` |",
        f"| gitlab_project_name | `{variables.get('gitlab_project_name', '')}` |",
        "",
        "## Generated Files",
        "",
        "```",
...
Examples:
  ...
  %(prog)s my_custom defaults --var gitlab_host=10.5.0.100
