...
Adapted from automation_v22/automation_library/build_stream/functions/pipeline_func.py
...
def get_catalog_content(host) -> Dict[str, Any]:
    ...
    check_cmd = run_in_container(host, f"test -f {catalog_file}")
    if check_cmd.rc != 0:
        result["error"] = f"Catalog file not found: {catalog_file}"
        return result

    cmd = run_in_container(host, f"cat {catalog_file}")
    if cmd.rc != 0:
...
def trigger_build_pipeline(host, log_callback=None) -> Dict[str, Any]:
    ...
    if not catalog_result["success"]:
        result["error"] = f"Failed to load catalog: {catalog_result['error']}"
        return result
    ...
    while time.time() - job_start_time < job_poll_timeout:
        ...
        if job_result["success"] and job_result["job_id"]:
            ...
            if new_job_id != old_job_id:
                ...
                _log(f"[{elapsed}s] Waiting for NEW job in DB...")
        else:
...
def trigger_deploy_pipeline(host, log_callback=None, use_pxe_commit: bool = True) -> Dict[str, Any]:
    ...
    if use_pxe_commit:
        ...
        result["pipeline_id"] = wait_result["pipeline_id"]
        result["details"] = f"Deploy pipeline {wait_result['pipeline_id']} auto-triggered via PXE commit"
    else:
        ...
        result["details"] = f"Deploy pipeline {trigger_result['pipeline_id']} triggered via PIPELINE_TYPE"
...
def select_image_for_deploy(host, pipeline_id: int, log_callback=None) -> Dict[str, Any]:
    ...
    if configured_id:
        ...
        for g in deployable_groups:
            if g.get("id") == configured_id:
                ...
                break
        if not target_group:
            result["error"] = f"Configured image_identifier '{configured_id}' not found"
            return result
    else:
    ...
    _log(f"Available selection jobs ({len(available_jobs)}): {available_jobs[:5]}")

    for job in jobs_result["jobs"]:
        if job.get("name", "") == image_group_id:
            target_job = job
            break

    if not target_job:
        if configured_id:
            result["error"] = f"Configured image_identifier '{configured_id}' not found in pipeline"
            return result
        ...
        if manual_jobs:
            ...
            result["image_group_id"] = target_job.get("name", "")
            for g in deployable_groups:
                if g.get("id") == result["image_group_id"]:
...
def trigger_cleanup_pipeline(host, log_callback=None) -> Dict[str, Any]:
    ...
    if running_pipelines:
        ...
        if allow_cancel:
            ...
            pipeline_ids = [str(p['id']) for p in running_pipelines]
            result["error"] = f"Pipeline(s) {', '.join(pipeline_ids)} are running/pending."
            return result
    ...
    if not cleanable_groups:
        result["error"] = "No cleanable image groups found."
        return result
    ...
    result["pipeline_id"] = trigger_result["pipeline_id"]
    result["details"] = f"Cleanup pipeline {trigger_result['pipeline_id']} triggered"
    result["success"] = True
    return result
...
def select_image_for_cleanup(host, pipeline_id: int, log_callback=None) -> Dict[str, Any]:
    ...
    if configured_id:
        ...
        target_group = sorted(candidates, key=lambda x: x.get("created_at", ""), reverse=True)[0]
        _log(f"Auto-selected: {target_group.get('id')} (status: {target_group.get('status')})")
            ...
            if "not found" in stage_result["error"].lower():
                ...
                if pipelines_result["success"] and pipelines_result["pipelines"]:
                    ...
                    if pipeline_status == "failed":
                        result["error"] = f"Pipeline #{pipeline['id']} failed."
                        return result
...
def monitor_pipeline_stages(
    host, job_id: str, stages: List[str],
