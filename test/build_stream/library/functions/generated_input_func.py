...
Adapted from automation_v22/automation_library/build_stream/functions/generated_input_func.py
...
from .shared_func import run_in_container, get_omnia_branch
...
def clone_omnia_repo(host, clone_path: str = DEFAULT_CLONE_PATH) -> Dict[str, Any]:
    """
    ...
    if not branch:
        result["error"] = (
            "omnia_branch is not set in test_config.yml. "
    ...
    run_on_host(host, f"rm -rf {clone_path}")

    clone_cmd = run_on_host(
        host,
    ...
    if clone_cmd.rc != 0:
        ...
        result["error"] = f"Failed to clone branch '{branch}': {out}"
        return result
...
def cleanup_omnia_clone(host, clone_path: str = DEFAULT_CLONE_PATH) -> None:
...
def get_software_config(host) -> Dict[str, Any]:
    ...
    if cmd.rc != 0 or not cmd.stdout.strip():
        result["error"] = f"Failed to read {SOFTWARE_CONFIG_PATH} from omnia_core"
        return result
...
def _find_config_file(
    ...
    if is_container:
        ls_cmd = run_in_container(host, f"ls {config_dir}/ 2>/dev/null")
    else:
        ls_cmd = run_on_host(host, f"ls {config_dir}/ 2>/dev/null")
...
def _compare_pkg_attrs(
    ...
    for key in compare_keys:
        ...
        if gen_val is None:
            ...
            lines.append(f"    VERSION MISMATCH: '{context}' / {pkg_name}")
            lines.append(f"      expected {key}: {src_val}")
            lines.append(f"      got {key}: none")
            result["issues"].append(
                f"'{context}': {pkg_name} '{key}' expected='{src_val}' got=none"
            )
        elif str(src_val) != str(gen_val):
            result["success"] = False
            lines.append(f"    VERSION MISMATCH: '{context}' / {pkg_name}")
            lines.append(f"      expected {key}: {src_val}")
            lines.append(f"      got {key}: {gen_val}")
            result["issues"].append(
                f"'{context}': {pkg_name} '{key}' expected='{src_val}' got='{gen_val}'"
            )


def compare_software_json(
    source_data: dict, generated_data: dict, software_name: str = "",
) -> Dict[str, Any]:
    """
    ...
    for role_name, source_groups in source_roles.items():
        if role_name not in generated_roles:
            result["success"] = False
            lines.append(f"    MISSING ROLE: '{role_name}'")
            result["issues"].append(f"role '{role_name}' missing in generated")
            continue
        ...
        for group_name, source_pkgs in source_groups.items():
            if group_name not in gen_groups:
                result["success"] = False
                lines.append(f"    MISSING GROUP: '{role_name}/{group_name}'")
                result["issues"].append(f"group '{role_name}/{group_name}' missing")
                continue
            ...
            for src_key, src_pkg in src_map.items():
                ...
                if src_key not in gen_map:
                    result["success"] = False
                    tag = src_pkg.get("tag", "")
                    display = f"{pkg_name} (tag: {tag})" if tag else pkg_name
                    lines.append(f"    MISSING PACKAGE: '{role_name}/{group_name}'")
                    lines.append(f"      expected: {display}")
                    result["issues"].append(f"missing '{display}' in '{role_name}/{group_name}'")
                    continue

                gen_pkg = gen_map[src_key]
                _compare_pkg_attrs(
                    src_pkg, gen_pkg, pkg_name,
                    f"{role_name}/{group_name}", result, lines,
                )
            ...
            for gen_key, gen_pkg in gen_map.items():
                if gen_key not in src_map:
                    ...
                    display = f"{pkg_name} (tag: {tag})" if tag else pkg_name
                    result["success"] = False
                    lines.append(f"    EXTRA PACKAGE: '{role_name}/{group_name}'")
                    lines.append(f"      got: {display}")
                    result["issues"].append(f"extra '{display}' in '{role_name}/{group_name}'")

    for role_name in generated_roles:
        if role_name not in source_roles:
            result["success"] = False
            lines.append(f"    EXTRA ROLE: '{role_name}'")
            result["issues"].append(f"extra role '{role_name}' in generated")
...
def verify_generated_inputs(
    ...
    if not os_type or not os_version:
        result["success"] = False
        result["error"] = "cluster_os_type or cluster_os_version not set in software_config.json"
        return result
    ...
    for software in softwares:
        ...
        for arch in archs:
            ...
            entry = _verify_single_software(
                host, sw_name, arch, os_type, os_version, source_base,
            )
            result["results"].append(entry)

            if entry.get("skipped"):
                skipped += 1
                all_lines.append(f"  [SKIP] {sw_name} ({arch}): {entry['details']}")
            elif entry["success"]:
                passed += 1
...
def _verify_single_software(
    ...
    gen_cmd = run_in_container(host, f"cat {gen_file} 2>/dev/null")
    src_cmd = run_on_host(host, f"cat {src_file} 2>/dev/null")
