...
def test_per_repo_policy_only(host: Host):
    ...
    if found_repo:
        ...
        tl.passed("other_configuration",
                 f"No repo with per-repo policy + global caching found among {len(configured_repos)} repos")
        pytest.skip("No repo has per-repo policy + global caching configuration")

    assert found_repo is not None, \
        f"Expected to find repo with per-repo policy + global caching"
...
def test_per_repo_caching_only(host: Host):
    ...
    if found_repo:
        ...
        tl.passed("other_configuration",
                 f"No repo with global policy + per-repo caching found among {len(configured_repos)} repos")
        pytest.skip("No repo has global policy + per-repo caching configuration")

    assert found_repo is not None, \
        f"Expected to find repo with global policy + per-repo caching"
...
def test_empty_per_repo_config(host: Host):
    ...
    if found_repo:
        ...
        tl.failed(LOG["empty_config_uses_global"],
                 f"No repo with global policy + global caching found among {len(configured_repos)} repos")
