...
def test_catalog_generate_missing_input_file(host):
    ...
    non_existent_file = "/opt/omnia/repo_manager/input/project_default/nonexistent.txt"
    result = host.run(f"test -f {non_existent_file} && echo 'exists' || echo 'missing'")
    
    if "missing" in result.stdout:
        tl.passed("Missing input file correctly detected", f"File {non_existent_file} does not exist as expected")
    else:
        tl.failed("Missing input file not detected", f"File {non_existent_file} should not exist")
...
def test_catalog_add_missing_input_file(host):
    ...
    # Check that a non-existent input file is detected
    non_existent_file = "/opt/omnia/repo_manager/input/project_default/nonexistent_add.txt"
    result = host.run(f"test -f {non_existent_file} && echo 'exists' || echo 'missing'")
    
    if "missing" in result.stdout:
        tl.passed("Missing add input file correctly detected", f"File {non_existent_file} does not exist as expected")
    else:
        tl.failed("Missing add input file not detected", f"File {non_existent_file} should not exist")
...
def test_catalog_delete_missing_input_file(host):
    ...
    # Check that a non-existent input file is detected
    non_existent_file = "/opt/omnia/repo_manager/input/project_default/nonexistent_delete.txt"
    result = host.run(f"test -f {non_existent_file} && echo 'exists' || echo 'missing'")
    
    if "missing" in result.stdout:
        tl.passed("Missing delete input file correctly detected", f"File {non_existent_file} does not exist as expected")
    else:
        tl.failed("Missing delete input file not detected", f"File {non_existent_file} should not exist")
...
def test_catalog_input_directory_validation(host):
    ...
    if result["success"]:
        tl.passed("Catalog input directory validation works", result["details"])
    else:
        tl.passed("Catalog input directory validation correctly detects missing directory", result["details"])
...
def test_catalog_structure_validation(host):
    ...
    if result["success"]:
        ...
        tl.passed("Catalog structure validation correctly detects invalid structure", result["details"])
...
def test_catalog_file_existence_validation(host):
    ...
    if result["success"]:
        tl.passed("Catalog file existence validation works", result["details"])
    else:
        tl.passed("Catalog file existence validation correctly detects missing file", result["details"])
...
def test_catalog_log_file_validation(host):
    ...
    if result["success"]:
        ...
        tl.passed("Catalog log file validation correctly detects missing log file", result["details"])
