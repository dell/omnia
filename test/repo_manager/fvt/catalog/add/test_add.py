...
def test_catalog_add_deploy(host):
    ...
    if "missing" in result.stdout:
        tl.failed("Catalog add input file not provided", 
                 ...
                 f"To run catalog_add tests, you must provide this file with packages/groups to add.\n"
                 f"Example format:\n"
                 f"[defaults]\n"
...
@pytest.mark.order(1)
def test_catalog_add_operation_completed(host):
    """TC_RM_CAT_ADD_001: Verify catalog add operation completed successfully."""
...
def test_catalog_packages_added_from_input_file(host):
    ...
    if not missing_packages:
        tl.passed("All packages from input file present in catalog", f"Present: {', '.join(expected_packages)}")
    else:
        tl.failed("Some packages from input file not found in catalog", f"Missing: {', '.join(missing_packages)}")
        assert False, f"Expected packages should be present: {', '.join(missing_packages)}"
...
def test_catalog_groups_created_from_input_file(host):
    ...
    if not missing_groups:
        tl.passed("All groups from input file present in catalog", f"Present: {', '.join(expected_groups)}")
    else:
        tl.failed("Some groups from input file not found in catalog", f"Missing: {', '.join(missing_groups)}")
        assert False, f"Expected groups should be present: {', '.join(missing_groups)}"
