...
def _list_profiles():
    ...
    for name, data in _available_profiles():
        ...
        print(f"  {_CYAN}{name:20s}{_NC} repo_config={repo_config}")
    print()
...
def _copy_from_src(output_dir):
    """Copy dataset files directly from src/ input directory."""
    ...
    # Copy JSON files
    for src_file in SRC_INPUT_DIR.glob("*.json"):
        dest = input_dir / src_file.name
        shutil.copy2(src_file, dest)
        _ok(f"Copied: input/{src_file.name}")

    copied = [str(p.relative_to(output_dir)) for p in sorted(output_dir.rglob("*"))]
    return copied
...
def _generate_readme(dataset_name, profile_name, variables, rendered, output_dir):
    ...
    lines = [
        ...
        f"| Repo config | `{variables.get('repo_config', '')}` |",
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  %(prog)s repo_manager_partial partial
  %(prog)s repo_manager_always always
  %(prog)s repo_manager_never never
  %(prog)s my_custom defaults --var pulp_server_port=2226
...
""",
    ...
    parser.add_argument(
        "profile",
        nargs="?",
        help="Profile name: defaults, partial, always, never, or a custom profile",
    )
    parser.add_argument(
        "--var",
        action="append",
    ...
    if args.from_src:
        _info("Copying from src/")
        rendered = _copy_from_src(output_dir)
        variables = {"repo_config": "from-src"}
