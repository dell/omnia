...
"""
Test Report Generator for validation/pytest tests.
...
def _donut_svg(passed: int, failed: int, skipped: int, size: int = 160) -> str:
    ...
    for color, count, label in palette:
        ...
        offset += dash
    # Pass rate = passed / (passed + failed) — skipped are not executed, so excluded
    executed = passed + failed
    pct = int(passed / executed * 100) if executed else (100 if skipped > 0 else 0)
    # The SVG container rotates -90deg so arcs start at top.
    # Text group counter-rotates +90deg to stay upright.
    return (
...
/* ── Responsive ── */
@media(max-width:920px){{.lay{{flex-direction:column}}.side{{width:100%}}.overview{{flex-direction:column;align-items:flex-start}}}}
</style>
</head>
<body>
<div class="container">
    ...
    if not servers:
        ...
        for sip, sd in servers.items():
            ...
            for run in reversed(runs):
                ...
                if rf: pills += f' <span class="pill pf">{rf} failed</span>'
                if rsk: pills += f' <span class="pill ps">{rsk} skipped</span>'
                ...
                for mi, mod in enumerate(modules):
                    ...
                    if mf: mpills += f' <span class="pill pf">{mf}</span>'
                    if msk: mpills += f' <span class="pill ps">{msk}</span>'
                    mid = f"{uid}-m{mi}"
                    ...
                    # Suite/marker/command meta tags
                    ...
                    # Split results into deploy vs verify
                    all_results = mod.get("results", [])
                    deploy_results = [r for r in all_results if "deploy" in r.get("test_name","").lower()
                    ...
                    if has_deploy:
                        ...
                        # Playbook logs inside Deploy section
                        if mod.get("playbook_logs"):
                            lid = f"{mid}-lg"
                            ct = (mod.get("command_type") or "execution").upper()
                            pf = False
                            lt = mod["playbook_logs"].lower()
                            for ind in ["failed=1","unreachable=1","fatal:","failed: [","playbook failed","deploy: failed"]:
                                if ind in lt:
                                    pf = True
                                    break
                            dc = "sf" if pf else "sp"
                            di = svg_x if pf else svg_check
                            html += (
                                f'<div class="plogs shut">'
                                f'<div class="plogs-h" onclick="togLogs(\'{lid}\')">'
                                f'<span class="plogs-arr">&#9654;</span>'
                                f'<span class="plogs-title">Playbook Logs ({ct})</span>'
                                f'<div class="sdot {dc}">{di}</div>'
                                f'</div>'
                                f'<div class="plogs-b" id="lg-{lid}" style="display:none">'
                                f'<pre>{_escape_html(mod["playbook_logs"])}</pre>'
                                f'</div></div>'
                            )
                        # Deploy test results
                        for test in deploy_results:
                            ...
                            if st == "PASSED": icls, isvg = "ip", svg_check
                            elif st == "SKIPPED": icls, isvg = "is", svg_skip
                            else: icls, isvg = "if", svg_x
                            html += _render_test_item(test_id, test, icls, isvg)
                        html += '</div></div>'  # dv-body, dv-sec
                    else:
                        # No deploy — show skipped
                        html += (
...
function togDV(id){document.getElementById('dv-'+id).classList.toggle('shut')}
function togT(e,id){e.stopPropagation();document.getElementById('t-'+id).classList.toggle('open')}
function togLogs(id){
  var c=document.getElementById('lg-'+id).parentElement;
...
class TestReport:
    ...
    def __init__(self, module_name: str, report_id: str = None):
        ...
        # Capture execution context from environment
        ...
        if log_file and os.path.exists(log_file):
            try:
                with open(log_file, 'r', encoding='utf-8') as f:
                    ...
                    # Split logs: only playbook execution (exclude pytest output)
                    test_start_markers = [
                        "test session starts",
                        "collecting ...",
                        "┌────────────────────────────────────────────────────────────────────┐"
