...
# =============================================================================
# Orchestrator — Validation Runner
# =============================================================================
set -euo pipefail
...
case "$SCENARIO" in
    ...
    help|--help|-h|"")
        echo -e "${BLUE}=================================================================${NC}"
        echo -e "${BLUE}  Orchestrator — Validation Runner${NC}"
        echo -e "${BLUE}=================================================================${NC}"
        echo ""
        echo "Usage:"
        ...
        echo "  --suite <name>    Filter by subfolder (openchami, status, etc.)"
        ...
        echo "  orchestrator     Full end-to-end (deploy all + verify)"
        echo "  validate         Verify input files exist on target"
        echo "  prepare          Deploy OpenCHAMI + verify containers"
        echo "  provision        Full provisioning (K8s, Slurm, OS)"
        echo "  cleanup          Cleanup + verify removal"
        echo ""
        ...
        echo ""
        echo "  # Validate inputs exist on target"
        echo "  $0 validate verify --marker sanity"
        echo ""
        echo "  # Deploy prepare and verify OpenCHAMI"
        echo "  $0 prepare test"
        echo ""
        echo "  # Verify only OpenCHAMI containers"
        echo "  $0 prepare verify --suite openchami"
        echo ""
        echo "  # Run cleanup and verify"
        echo "  $0 cleanup test"
        echo ""
