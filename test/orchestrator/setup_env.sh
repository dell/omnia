...
DOMAIN_CREDS_FILENAME="orchestrator_credentials.yml"
...
─────────────────────────────────────────────────────────────────
  Created on this machine at:
    $ORCHESTRATOR_DATA_PATH/input/$OMNIA_PROJECT_NAME/, when set;
    otherwise $OMNIA_DATA_PATH/orchestrator/input/$OMNIA_PROJECT_NAME/.
  Fields: provision_password, bmc_username, bmc_password, slurm_db_password,
          openldap_db_username, openldap_db_password, csi_username, csi_password.
...
if [ "$DOMAIN_CREDS_FROM_STDIN" = true ]; then
    ...
    if [ "$UPDATE_DOMAIN_CREDS" = true ] || [ "$SET_DOMAIN_CREDS" = true ]; then
        echo ""
        echo -e "  ${CYAN}Orchestrator Credentials — Provision, Slurm, OpenLDAP, CSI${NC}"
...
echo ""
echo "    2. Orchestrator domain credentials (current machine):"
