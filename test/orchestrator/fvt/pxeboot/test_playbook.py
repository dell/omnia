...
"""
Orchestrator PXE Boot — Deploy.

TC_PX_000: Deploy orchestrator.yml --tags pxeboot
...
def test_deploy_pxeboot(host):
    """TC_PX_000: Deploy orchestrator.yml --tags pxeboot."""
    tl = TestLogger(
        TEST_NAMES["deploy_playbook"].format(tag="pxeboot"), "TC_PX_000"
    )
    ...
    assert result["success"], ASSERT["playbook_failed"].format(
        playbook="orchestrator.yml", tag="pxeboot",
        rc=result["rc"], duration=result["duration"],
    )
