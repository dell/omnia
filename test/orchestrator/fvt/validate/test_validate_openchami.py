...
def test_orchestrator_config_exists(host):
    ...
    result = host.file(config_path).exists
    
    if result:
        tl.passed(f"orchestrator_config.yml present", 
                 f"orchestrator_config.yml found at {config_path}")
    else:
        tl.failed("orchestrator_config.yml missing",
                 f"orchestrator_config.yml not found at {config_path}")
    
    assert result, f"orchestrator_config.yml not found at {config_path}"
...
def test_pxe_mapping_file_exists(host):
    ...
    result = host.file(pxe_path).exists
    
    if result:
        tl.passed(f"pxe_mapping_file.csv present",
                 f"pxe_mapping_file.csv found at {pxe_path}")
    else:
        tl.failed("pxe_mapping_file.csv missing",
                 f"pxe_mapping_file.csv not found at {pxe_path}")
    
    assert result, f"pxe_mapping_file.csv not found at {pxe_path}"


@pytest.mark.functional
@pytest.mark.order(3)
def test_orchestrator_config_valid_yaml(host):
