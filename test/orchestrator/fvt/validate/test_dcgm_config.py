...
def test_dcgm_enabled_in_schema():
    ...
    schema_path = "/share_sayuri/sujit/omnia/src/orchestrator/plugins/module_utils/orchestrator_validation/schema/orchestrator_config.json"
    with open(schema_path) as f:
        schema = json.load(f)
    
    assert "dcgm_enabled" in schema["properties"], "dcgm_enabled should be in schema properties"
    assert schema["properties"]["dcgm_enabled"]["type"] == "boolean", "dcgm_enabled should be boolean type"
    assert schema["properties"]["dcgm_enabled"]["default"] is True, "dcgm_enabled should default to true"
...
def test_dcgm_enabled_in_input_file():
    ...
    assert "dcgm_enabled" in config, "dcgm_enabled should be in orchestrator_config.yml"
    assert config["dcgm_enabled"] is True, "dcgm_enabled should default to true in input file"
