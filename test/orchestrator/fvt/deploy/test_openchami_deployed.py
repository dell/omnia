...
@pytest.mark.order(1)
def test_openchami_deployed(host):
    ...
    if result.rc == 0:
        ...
        openchami_containers = [c for c in containers if 'openchami' in c.lower() or 
                               'smd' in c.lower() or 'bss' in c.lower() or 
                               'tokensmith' in c.lower() or 'hydra' in c.lower()]
        
        if openchami_containers:
            tl.passed(f"OpenCHAMI containers deployed and running",
                     f"Found {len(openchami_containers)} OpenCHAMI containers: {', '.join(openchami_containers[:5])}")
        else:
...
def test_openchami_services_active(host):
    ...
    if active_services:
        tl.passed(f"OpenCHAMI services deployed and active",
                 f"Found {len(active_services)} active services: {', '.join(active_services)}")
    else:
