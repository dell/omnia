...
class TestInputFilesExist:
    ...
    def test_functional_group_packages_exists(self, repo_manager_output):
        ...
        assert (repo_manager_output / "functional_group_packages.yml").exists()

    def test_certs_dir_exists(self, repo_manager_output):
        """certs/ directory must exist."""
        assert (repo_manager_output / "certs").is_dir()

    def test_config_sample_exists(self, repo_root):
        """config.yml.sample must exist."""
        assert (repo_root / "config.yml.sample").exists()
