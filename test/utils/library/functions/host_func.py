...
def get_slurm_config_util_output_path(host) -> str:
    """Get the default Slurm config util backup output path on target.

    Reads OMNIA_DATA_PATH and OMNIA_PROJECT_NAME from the target's
    environment to resolve the default slurm_config_util workspace
    (used when OMNIA_BACKUP_PATH is not set).
    ...
    try:
        data_path = read_remote_env(host, ENV_OMNIA_DATA_PATH)
        project = read_remote_env(host, ENV_OMNIA_PROJECT_NAME)
        return f"{data_path}/{DOMAIN_NAME}/output/{project}/slurm_config_util"
    except Exception:
        return ""


def get_slurm_config_util_config_path(host) -> str:
    """Get the slurm_config_util_config.yml path on target.
    ...
    try:
        input_path = get_utils_input_path(host)
        return f"{input_path}/{SLURM_CONFIG_UTIL_CONFIG_FILE}"
    except Exception:
        return ""
