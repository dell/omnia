...
def get_json_file_path(software_name, cluster_os_type, cluster_os_version, user_json_path):
    """Construct the JSON file path for the given software."""
...
def get_failed_software(file_name):
    """Retrieve a list of software names with a 'failed' status from a CSV file."""
    failed_software = []
...
def parse_json_data(file_path, package_types, failed_list=None, subgroup_list=None):
    """
    Parse the JSON data and return a list of filtered packages.
    Filters are applied based on package type and failed package list.
    """
...
def check_csv_existence(software, path):
    """Check if the software's CSV file exists."""
    return os.path.isfile(path)
 
def process_software(software, fresh_installation, json_path, csv_path, subgroup_list):
    """
    Process a software entry by checking its status,
    parsing its JSON data, and returning relevant tasks.
    """
...
def get_software_names(data_path):
    """Extract software names from JSON."""
